...
public slots:
	void loadSettings();
	bool init();
	bool setDeviceFile(QString p);
	bool setBaudRate(int b);
	bool setResponseTimeout(int t);
	void send(QByteArray du);
private slots:
	void readData();
private:
	...
	QByteArray read_buf;
	QList<QByteArray> send_buf;
	QList<QByteArray> routine_buf;
	int response_timeout;
	int frame_timeout;
	int unresponsed_data_units;
	int max_unresponsed_data_units;
	volatile bool write_lock;
	volatile bool routine_flag;
	int reinit_count;
	*/

public slots:
	static quint16 crc16(QByteArray);
	static bool    checkDataUnit(QByteArray);

	static quint8 getID(QByteArray);
	static quint8 getFnCode(QByteArray);
	static QByteArray getData(QByteArray);

	static qint32  decodeInt32 (QByteArray);
	static quint32 decodeUInt32(QByteArray);
	static qint16  decodeInt16 (QByteArray);
	static quint16 decodeUInt16(QByteArray);
	static float   decodeFloat (QByteArray);
	static QString decodeString(QByteArray);
	...
	static QByteArray dataUnit(quint8, quint8, QByteArray);
	static QByteArray dataUnit(quint8, quint8);
	static QString toString(QByteArray);
